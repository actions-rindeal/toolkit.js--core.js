...
class Core {
   ...
   */
  static getMultilineInput(name, required) { return this.getInput(name, required).split('\n').filter(x => x).map(input => input.trim()) }
  /**
  ...
  static getBooleanInput(name, required) {
    ...
    return { 'true': true, 'false': false }[val] ?? (() => { throw new TypeError(`Input "${name}"="${val}" is not a valid boolean.`) })()
  }
  /**
  ...
  static exportVariable(name, val) {
    process.env[name] = this.toCommandValue(val)
    return this.#issueFileCommand('ENV', this.#prepareKeyValueMessage(name, val))
  }
  /**
   * Masks a secret in logs.
   * @param {string} secret - The secret to mask.
   */
  static setSecret(secret) { this.#issueCommand('add-mask', {}, secret) }
  /**
   * Adds a path to the PATH environment variable.
   * @param {string} inputPath - The path to add.
   */
  static addPath(inputPath) { this.#issueFileCommand('PATH', inputPath) ; process.env.PATH = `${inputPath}${path.delimiter}${process.env.PATH}` }
  /**
  ...
  static #issueCommand(command = 'missing.command', properties = {}, message = '') {
    const propStr = Object.entries(properties)
      ...
      .join(',')
    process.stdout.write(`::${command} ${propStr}::${this.#escapeData(message)}${os.EOL}`)
  }
  ...
  static #escapeProperty(s) { return this.#escapeData(s).replace(/:/g, '%3A').replace(/,/g, '%2C') }
